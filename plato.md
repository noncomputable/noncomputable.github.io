...
I was rattled. How didn't they figure this out before implementing it? Why not much sooner than nearly a year after the fact? Their final report pointed back to a [paper from 2012](https://journals.sagepub.com/doi/abs/10.1177/1529100612451018?journalCode=psia) on the psychology of misinformation for suggestions as to why it didn't work out.

If they couldn't diligently decide whether to apply a feature like this, when the stakes were so high for both the public's epistemic welfare and their corporate reputation, could they reliably administer something like a [full VR world of billions of people](https://www.wsj.com/articles/facebook-sets-goal-of-a-billion-virtual-reality-users-unveils-new-headset-1507764852)? Where tiny architectural tweaks could have massive, harmful effects? 

How can they be more reliable? Reading up on prior research _before_ proliferating big interface changes is a start. Innocuous features, like reversing the order of text, might have big, [documented effects](https://onlinelibrary.wiley.com/doi/full/10.1002/jcpy.1053). But that's also shaky. The 2012 paper Facebook cited in their retrospective referenced an earlier, general result on the backfire effect; in 2018, a [larger study](https://link.springer.com/article/10.1007/s11109-018-9443-y) failed to replicate that result among others. Due at least partly to [perverse incentives in academic publishing](https://journals.plos.org/plosmedicine/article?id=10.1371/journal.pmed.0020124), that a social or behavioral result is documented in a journal [doesn't appear to be a very strong signal that it's true](https://www.nature.com/articles/s41562-018-0399-z). 

But this isn't fatalistic. If researchers have a serious stake in the accuracy of their results, like those at big, influential firms should, they can design more reliable studies [for the specific features they're proposing](https://link.springer.com/article/10.1007/s11109-019-09533-0) and to report more accurate results. Results that suggest how interface features might affect users' wellbeing.

"Wellbeing" is vague. Optimizing for a handful of "wellbeing" metrics can distract from less easily measurable objectives involving users' [reasons & values](https://medium.com/what-to-build/is-anything-worth-maximizing-d11e648eb56f). Yet there are some measures of wellbeing that I think typically should influence (but not _determine_) how interfaces change and features get proliferated.

[For online marketplaces](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.22.7276&rep=rep1&type=pdf), Jungpil Hahn suggested studying the correlation between different interface features and bidding activity (i.e. matches between people looking for things and people offering things). For social media, [Kramer, Guillory, and Hancock](https://www.pnas.org/content/pnas/111/24/8788.full.pdf) studied how different news feed features affected the diffusion of positive and negative emotions on Facebook. Along those lines, [Rose-Stockwell](https://qz.com/1264547/facebooks-problems-can-be-solved-with-design/) suggests measuring how interface features fuel polarization and vitriol and designing for empathy.

Beyond happiness and health is _epistemic wellbeing_. How does the system, or the interface through which users access it, affect their beliefs. Along these lines, [Bode and Vraga](https://onlinelibrary.wiley.com/doi/abs/10.1111/jcom.12166) studied how displaying related links next to misinformation could affect users' attitudes towards it. They tried to figure out which features caused which people to change their beliefs and in which direction.

I'm interested in how interacting with a system affects people's ability to make good, informed decisions. Relatedly, [Seuken et al.](https://www.ifi.uzh.ch/ce/publications/BehavioralFactorsInMarketUIDesign.pdf) studied the effects of interface features on decisionmaking where users are explicitly told the value and price of each item in a menu and have a limited amount of money to spend. 
...
If we have users and an interface, how can we measure its information adversity?

Measures of information adversity will assume that users' decisions are based on their beliefs, with room for violations (e.g. accidental movements of the hand). What fits this pretty well? Consumption! Whether it's a physical product, an entertainment service, or transportation, people's consumption decisions are largely based on their beliefs.

We can run an experiment that goes like this: After injecting information about several sets of products into the system under study, we randomly assign each user an interface through which they can access the system. After interacting with it for some constant amount of time, they're given a series of menus and allowed to nominate products from each set of products, along with an incentive tied to the quality of each nomination. Having recorded their decisions, we can calculate and compare the information adversity of different interfaces.
...
We inject messages carrying information about the products into the system we're studying and let it expose them according to the rules of its interface. That can take the form of Facebook posts, Twitter bios, eBay listings, Amazon reviews, Google Sheets rows, Minecraft signposts, and so on. To control for the effects of different distributions of accuracy, we ought to generate messages so that their accuracy varies randomly across users & treatments.

With so much information to expose, most users probably won't see these messages (i.e. tweeting into the void). Messages need to have certain properties to have a chance of being seen by the users we're studying, like being authored by someone they're connected to or being placed in a game region they frequent. 

I want to know the information adversity of an interface independent of how these properties are distributed among the messages we inject. If we model the relationship between different properties a message can have and the likelihood that users will see it, we can craft messages with properties to vary their likelihood of exposure. We should randomly distribute these properties across messages to control for their exposure effects and isolate the effects of the interface features we're studying.

Once info about our artificial products is in the system, we can let each user go at their designated interface.

We want to know which products they _think_ are the best—limited by what the interface has shown them—and compare them to the ones that are actually the best. Would we just ask them about their preferences? If users see a survey pop up, would they answer it as if they were in a situation with a real stake in their decisions? Our premise was that there are situations where users actually make decisions based on their beliefs, and we could name one of them: consumption. How can we get the subjects to act as consumers?

We tell them that their decisions won't only provide us information, but will provide them with products. We'll give them one buying credit for each substitute set, tell them to nominate their top products in $$ S$$, and that the credit will be randomly spent on one of their nominations.

If they understand the product will be picked randomly from their nominations, the prospective value of their decision (formally, its [subjective expected utility](https://en.wikipedia.org/wiki/Subjective_expected_utility)) will be the average of the prospective qualities of all their individual nominations. So we'd expect them to only nominate the products they think have the highest quality—though [in some settings people don't behave like this](https://en.wikipedia.org/wiki/Ellsberg_paradox), so we can't take this assumption for granted. Nominating products with lower prospective qualities than the maximum available will just lower the total prospective value of their decision, and so they'll only have an incentive to nominate the ones with the highest prospective quality in $$ S$$, i.e. their top products.
